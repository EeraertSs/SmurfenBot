...
TASKS = []
START_STATS = {}
TASK_POOL_FILE = ".../config/task_pool_v4.json"
...
async def post_weekly_tasks(manual=False):
    ...
    # ✅ Filter: enkel taken die cooldown hebben overleefd
    valid_tasks = []
    for task in task_pool['tasks']:
        if task['last_picked']:
            last_picked_date = datetime.datetime.strptime(task['last_picked'], "%Y-%m-%d")
            weeks_since = (now - last_picked_date).days // 7
            if weeks_since >= task.get('cooldown_weeks', 0):
                valid_tasks.append(task)
        else:
            valid_tasks.append(task)  # Nog nooit gekozen = altijd geldig

    if len(valid_tasks) < 5:
        await channel.send("⚠️ Niet genoeg beschikbare tasks! Vul je task_pool.json verder aan.")
        return

    # ✅ Selecteer random 5 geldige taken
    selected_tasks = random.sample(valid_tasks, 5)

    # ✅ Verwerk geselecteerde taken
    TASKS = []
    ...
    # ✅ Save task pool (update last_picked!)
    ...
    for task in TASKS:
        if task['type'] == 'drop':
            ...
            embed.add_field(name="📚 EXP Task", value=f"**Gain {task['amount']} {task['skill']} EXP**", inline=False)
        elif task['type'] == 'bosskc':
            embed.add_field(name="⚔️ Boss Killcount Task", value=f"**Kill {task['boss']} {task['amount']} times**", inline=False)
        elif task['type'] == 'clue':
            embed.add_field(name="🗺️ Clue Scroll Task", value=f"**Complete {task['amount']} {task['tier'].capitalize()} clues**", inline=False)
    ...
    # ✅ Start stats ophalen voor progress tracking
...
async def daily_progress_update_loop():
    ...
    # Progressbar tonen na dagelijkse progressie update
    if not TASKS:
        await channel.send("⚠️ Er zijn momenteel geen actieve tasks.")
        return
    ...
    embed = discord.Embed(
        title="📊 Dagelijkse Smurfen Progressie",
...
async def starttestweek(ctx):
    ...
    TASKS = [
        {"type": "exp", "skill": "Fishing", "amount": 50, "gained": 0, "category": "Test", "player_progress": {}},
        {"type": "exp", "skill": "Woodcutting", "amount": 50, "gained": 0, "category": "Test", "player_progress": {}},
        {"type": "exp", "skill": "Fletching", "amount": 50, "gained": 0, "category": "Test", "player_progress": {}}
    ]
...
async def update_progress(ctx=None, check_daily=False):
    ...
    for task in TASKS:
        if task["type"] == "drop":
            icon = "✅" if task.get("completed", False) else "🟠"
            type_icon = "🧤"
            if task.get("completed", False):
                completed_tasks += 1
            embed.add_field(name=f"{icon} {type_icon} {task['description']}", value="Drop Task", inline=False)
            ...
            # ➔ Player breakdown erbij zetten
            player_progress_text = ""
            for player, exp in player_gains.items():
                if exp > 0:
                    player_progress_text += f"🪓 {player}: {exp:,} EXP\n"
            if not player_progress_text:
                player_progress_text = "No contributions yet."

            embed.add_field(
                name=f"{icon} {type_icon} Gain {task['amount']} {task['skill']} EXP",
                value=f"{gained:,} / {task['amount']:,} EXP ({perc:.1f}%)\n\n{player_progress_text}",
                inline=False
            )
            ...
            total_kills = 0
            for player in PLAYERS:
                start_kc = START_STATS[player]["bosses"].get(task["boss"].replace("_", " ").title(), 0)
                now_kc = current_stats[player]["bosses"].get(task["boss"].replace("_", " ").title(), 0)
                kills = max(now_kc - start_kc, 0)
                total_kills += kills

            if total_kills >= task["amount"]:
                icon = "✅"
                completed_tasks += 1
            elif total_kills > 0:
                icon = "🟠"
                ...
                icon = "❌"
            type_icon = "⚔️"
            embed.add_field(name=f"{icon} {type_icon} Kill {task['boss']} {task['amount']} times", value=f"{total_kills}/{task['amount']} KC", inline=False)

        elif task["type"] == "clue":
            total_clues = 0
            tier_key = f"Clue Scrolls {task['tier'].capitalize()}"
            for player in PLAYERS:
                start_clues = START_STATS[player]["skills"].get(tier_key, 0)
                now_clues = current_stats[player]["skills"].get(tier_key, 0)
                clues_done = max(now_clues - start_clues, 0)
                total_clues += clues_done

            if total_clues >= task["amount"]:
                icon = "✅"
                completed_tasks += 1
            elif total_clues > 0:
                icon = "🟠"
                ...
                icon = "❌"
            type_icon = "🗺️"
            embed.add_field(name=f"{icon} {type_icon} Complete {task['amount']} {task['tier'].capitalize()} Clues", value=f"{total_clues}/{task['amount']}", inline=False)
...
async def progressbar(ctx):
    ...
    # ➡️ Daarna progressbar berekenen
    percent = int((completed / total_tasks) * 100)
    filled = int(percent / 5)  # 20 blokjes totaal
    bar = "█" * filled + "—" * (20 - filled)
